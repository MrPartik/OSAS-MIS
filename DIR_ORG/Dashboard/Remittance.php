<?php
    ...
    $view_query = mysqli_query($con,"SELECT  SUM(OrgRemittance_AMOUNT) AS AMOUNT,DATE_FORMAT(OrgRemittance_DATE_ADD, '%Y') AS DATE FROM t_org_remittance
                                                    ...
                                                    WHERE OrgRemittance_DISPLAY_STAT = 'Active' AND OrgForCompliance_DISPAY_STAT = 'Active' AND OrgAppProfile_DISPLAY_STAT = 'Active' AND OrgForCompliance_OrgApplProfile_APPL_CODE = (SELECT OrgForCompliance_OrgApplProfile_APPL_CODE FROM t_org_for_compliance WHERE OrgForCompliance_ORG_CODE = '$appcode') GROUP BY DATE_FORMAT(OrgRemittance_DATE_ADD, '%Y') ORDER BY OrgRemittance_DATE_ADD ASC ");
    $container_arr = array();
