<?php
	...
	include('../../../config/connection.php');
        ...
        $view_query = mysqli_query($con,"SELECT COUNT(*) AS COU,OrgForCompliance_ORG_CODE FROM `t_org_for_compliance` WHERE OrgForCompliance_DISPAY_STAT = 'Active' AND OrgForCompliance_OrgApplProfile_APPL_CODE = '$appcode'");
        ...
        if($count == '0')
        {
             $query = mysqli_query($con,"INSERT INTO t_org_for_compliance (OrgForCompliance_ORG_CODE,OrgForCompliance_OrgApplProfile_APPL_CODE,OrgForCompliance_BATCH_YEAR,OrgForCompliance_ADVISER) 
                                                VALUES ('$compcode','$appcode','$year',null)  ");
            
        }
        else
        {
             $query = mysqli_query($con,"UPDATE t_org_for_compliance SET OrgForCompliance_BATCH_YEAR = '$year'
                                        WHERE OrgForCompliance_ORG_CODE =  '$compcode' ");
            
        }
       
        $view_query = mysqli_query($con,"SELECT COUNT(*) AS COU FROM `r_application_wizard` WHERE WIZARD_ORG_CODE = '$compcode'");
        while($row = mysqli_fetch_assoc($view_query))
        {   
            $count  = $row["COU"];
