<?php
    ...
    $view_query = mysqli_query($con,"SELECT OrgOffi_OrgOffiPosDetails_ID posID, AssOrgMem_STUD_NO studno, CONCAT(Stud_LNAME,', ',Stud_FNAME ,IF(Stud_MNAME = '','',CONCAT(' ',Stud_MNAME)))  AS NAME , Stud_NO,CONCAT(Stud_COURSE,' ',Stud_YEAR_LEVEL,' - ',Stud_SECTION) AS CAS,OrgOffiPosDetails_NAME AS POS FROM t_assign_org_members
    ...
    while($row = mysqli_fetch_assoc($view_query))
    {
        ...
        $posID = $row["posID"];
        $studno = $row["studno"];
        $arr = array('name' => $name,'pos' => $pos,'studno' =>$studno,'posID'=>$posID );
