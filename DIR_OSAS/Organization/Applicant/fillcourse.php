<?php
    ...
    $view_query = mysqli_query($con,"SELECT AssOrgAcademic_COURSE_CODE as COURSE FROM `t_assign_org_academic_course`  INNER JOIN r_courses ON Course_CODE = AssOrgAcademic_COURSE_CODE
                where AssOrgAcademic_ORG_CODE IN (SELECT OrgForCompliance_ORG_CODE FROM t_org_for_compliance WHERE OrgForCompliance_OrgApplProfile_APPL_CODE = '$id') AND AssOrgAcademic_DISPLAY_STAT = 'Active' AND Course_DISPLAY_STAT = 'Active' ") ;
    $container_arr = array();
