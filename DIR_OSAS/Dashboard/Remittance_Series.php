<?php
    ...
    $view_query = mysqli_query($con,"SELECT Batch_YEAR,sum(OrgRemittance_AMOUNT) AS AMO, OrgForCompliance_ORG_CODE FROM `r_batch_details` 
                                            ...
                                            WHERE OrgRemittance_DISPLAY_STAT = 'Active' AND OrgAppProfile_DISPLAY_STAT = 'Active' AND OrgRemittance_APPROVED_STATUS = 'Approved' AND Batch_DISPLAY_STAT = 'Active' GROUP BY Batch_YEAR");
    $container_arr = array();
    ...
    while($row = mysqli_fetch_assoc($view_query))
    {
        ...
        INNER JOIN r_batch_details ON Batch_YEAR = OrgForCompliance_BATCH_YEAR
		    WHERE OrgRemittance_DISPLAY_STAT = 'Active' AND OrgAppProfile_DISPLAY_STAT = 'Active' AND  OrgRemittance_APPROVED_STATUS = 'Approved' AND OrgForCompliance_BATCH_YEAR = '$ser' GROUP BY OrgForCompliance_ORG_CODE ");
        while($row2 = mysqli_fetch_assoc($view_query2))
        {
            ...
            INNER JOIN r_org_applicant_profile ON OrgForCompliance_OrgApplProfile_APPL_CODE = OrgAppProfile_APPL_CODE
            	WHERE OrgRemittance_DISPLAY_STAT = 'Active' AND OrgAppProfile_DISPLAY_STAT = 'Active' AND OrgRemittance_APPROVED_STATUS = 'Approved' AND OrgRemittance_ORG_CODE = '$code' ");
