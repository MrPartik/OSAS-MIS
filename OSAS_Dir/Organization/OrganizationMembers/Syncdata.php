...
//    $query = mysqli_query($connection,"UPDATE t_assign_org_members SET AssOrgMem_DISPLAY_STAT = 'Inactive' WHERE AssOrgMem_APPL_ORG_CODE = '$compcode' ");
    $view_query = mysqli_query($connection," SELECT CONCAT(Stud_LNAME,' ,',Stud_FNAME ,' ', IFNULL(Stud_MNAME,'')) AS NAME , Stud_NO,OrgForCompliance_OrgApplProfile_APPL_CODE FROM `r_stud_profile` 
    ...
    while($row = mysqli_fetch_assoc($view_query))
    {
        ...
        $getcount = mysqli_query($connection," SELECT COUNT(*) AS COU FROM t_assign_org_members WHERE AssOrgMem_STUD_NO = '$no' AND AssOrgMem_APPL_ORG_CODE = '$code' ");
