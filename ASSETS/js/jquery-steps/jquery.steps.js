...
(function ($, undefined) {
    ...
    function finishStep(wizard, state) {
        ...
        $.ajax({
            url: "../DIR_OSAS/studProfileModal.php?StudID=11"
            , cache: false
            , async: false
        ...
        if (state.stepCount > 0) {
            ...
            if (!options.enableContentCache || !currentStep.contentLoaded) {
                ...
                case contentMode.async:
                    var currentStepContent = getStepPanel(wizard, state.currentIndex)._aria("busy", "true").empty().append(renderTemplate(options.loadingTemplate, {
    ...
    function refreshSteps(wizard, options, state, index) {
        ...
        for (var i = index; i < state.stepCount; i++) {
            ...
            wizard.find(".steps a").eq(i)._setId(uniqueStepId)._aria("controls", uniqueBodyId).attr("href", "#" + uniqueHeaderId).html(renderTemplate(options.titleTemplate, {
                index: i + 1
                , title: title.html()
            }));
            wizard.find(".body").eq(i)._setId(uniqueBodyId)._aria("labelledby", uniqueHeaderId);
        }
    }
    ...
    function renderBody(wizard, body, index) {
        ...
        body._setId(uniqueBodyId).attr("role", "tabpanel")._aria("labelledby", uniqueHeaderId).addClass("body")._hideAria();
    }
    /**
    ...
    function renderPagination(wizard, options, state) {
        if (options.enablePagination) {
            var pagination = "<{0} class=\"actions {1}\"><ul role=\"menu\" aria-label=\"{2}\">{3}</ul></{0}>"
                , buttonTemplate = "<li><a data-toggle='modal' href=\"#{0}\" role=\"menuitem\">{1}</a></li>"
                , buttons = "";
            if (!options.forceMoveForward) {
            ...
            wizard.append(format(pagination, options.actionContainerTag, options.clearFixCssClass, options.labels.pagination, buttons));
            refreshPagination(wizard, options, state);
            loadAsyncContent(wizard, options, state);
        }
    }
